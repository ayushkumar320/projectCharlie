...
export const LampContainer = ({children, className}) => {
  return (
    <div
      className={cn(
        "relative z-30 flex min-h-screen flex-col items-center justify-center overflow-hidden bg-transparent w-full rounded-md",
        className
      )}
    >
      {/* Soft blur at the top to blend with previous section without blurring entire lamp */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-56 sm:h-64 md:h-72 bg-linear-to-b from-slate-900/20 via-slate-900/10 to-transparent backdrop-blur-md z-0" />
      {/* Full-section soft blur to match initiatives section without washing out beams */}
      <div className="pointer-events-none absolute inset-0 bg-slate-900/5 sm:bg-slate-900/10 backdrop-blur-sm z-0" />
      <div className="relative flex w-full flex-1 scale-y-150 md:scale-y-125 items-center justify-center isolate z-10 translate-y-24 sm:translate-y-28 md:translate-y-36 lg:translate-y-44 xl:translate-y-48">
        ...
        >
          <div className="absolute w-full left-0 bg-slate-950/20 backdrop-blur-sm h-40 bottom-0 z-20 mask-[linear-gradient(to_top,white,transparent)]" />
          <div className="absolute w-40 h-full left-0 bg-slate-950/20 backdrop-blur-sm bottom-0 z-20 mask-[linear-gradient(to_right,white,transparent)]" />
        </motion.div>
        <motion.div
          initial={{opacity: 0.5, width: "12rem"}}
          whileInView={{opacity: 1, width: "25rem"}}
        ...
        >
          <div className="absolute w-40 h-full right-0 bg-slate-950/20 backdrop-blur-sm bottom-0 z-20 mask-[linear-gradient(to_left,white,transparent)]" />
          <div className="absolute w-full right-0 bg-slate-950/20 backdrop-blur-sm h-40 bottom-0 z-20 mask-[linear-gradient(to_top,white,transparent)]" />
        </motion.div>
        <div className="absolute top-1/2 h-48 w-full translate-y-12 scale-x-150 bg-slate-950/20 backdrop-blur-sm blur-2xl"></div>
        ...
        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-50 bg-slate-950/20 backdrop-blur-sm"></div>
      </div>
